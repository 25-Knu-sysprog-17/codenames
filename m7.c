...
void handle_dynamic_input(char *buffer, size_t size, int y, int x) {
    ...
    while (1) {
        ...
        else if (ch == 9) { // Tab → 채팅모드
            input_mode = INPUT_CHAT;
            return;
        } else if (ch == 1) { // Ctrl+A → 신고모드
        ...
        } else if (ch >= 32 && ch <= 126 && len < size - 1) {
            buffer[len++] = ch;
            buffer[len] = '\0';
...
void handle_chat_input(int y, int x, char *name) {
    ...
    while (1) {
        ...
        if (ch == 10) break;
            ...
            if (chat_scroll_offset > 0)
                chat_scroll_offset--;
            return;
        } else if (ch >= 32 && ch <= 126 && len < MAX_INPUT_LEN - 1) {
            chat_input[len++] = ch;
            chat_input[len] = '\0';
            mvaddch(y, x + 10 + len - 1, ch);
            move(y, x + 10 + len);
        }
        refresh();
    }
    ...
    if (strlen(chat_input) > 0) {
        char msg[128];
        snprintf(msg, sizeof(msg), "%s: %s", name, chat_input);
